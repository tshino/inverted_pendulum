...
USE_BFGS_OPTIMIZER = True   # use BFGS, otherwise use Adam
...
def calc_state_loss(state):
  ...
  return dist_a * tf.math.rsqrt(dist_b + 0.0001)
...
def run_optimization(initial_gain):
  ...
  if USE_BFGS_OPTIMIZER:
    ...
    def optimize():
      return tfp.optimizer.lbfgs_minimize(
        ...
        initial_position=tf.constant(np.array(initial_gain)),
        tolerance=1e-5
      )
  else:
  ...
  print(' last state =', last_state.numpy())
